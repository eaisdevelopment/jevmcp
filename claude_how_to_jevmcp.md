<!-- Generated by the maintainers' scripts/build_howto.py from the shipped plugin, jevmcp 1.7.0.
     Verbatim sources:
       plugins/jevmcp/skills/spec-drift/SKILL.md (sha256 9e046879687110e0...),
       plugins/jevmcp/skills/ci-triage/SKILL.md (sha256 14f40ef5ff1e5f77...),
       plugins/jevmcp/skills/code-audit/SKILL.md (sha256 b09eaa2a9e06e1a7...),
       plugins/jevmcp/scripts/jevmcp_server.py (INSTRUCTIONS and TOOLS; sha256 c79aba5827630574...).
...
- `spec-drift`: `9e046879687110e0eae99278242941ce032488222374a8e190f0e131b8de97e3`
- `ci-triage`: `14f40ef5ff1e5f779c57130d5bc660a332bbf2995091af3dde5e6218c7b2c764`
...
| 3 | Each skill's body — the procedure | spec-drift 11.5 kB, ci-triage 9.1 kB, code-audit 10.8 kB | only when that skill fires |
| 4 | The 10 tools' descriptions and JSON schemas | 31,536 characters in all | at call time |
...
### 3.1 `spec-drift` — `plugins/jevmcp/skills/spec-drift/SKILL.md` (sha256 `9e046879687110e0...`, 12,327 bytes)
...
### 3.2 `ci-triage` — `plugins/jevmcp/skills/ci-triage/SKILL.md` (sha256 `14f40ef5ff1e5f77...`, 9,887 bytes)
...
Structured result fields: `summary`, `project`, `map`, `claims_in_map`, `claims_selected`, `checked`, `counts`, `cost_usd`, `complete`, `results_file`, `map_problems`, `not_checked`, `map_health`, `flagged`.
...
Structured result fields: `project`, `map`, `ready`, `entries_to_check`, `excluded`, `full_check_cost_usd`, `full_check_cost_usd_max`, `samples`, `likely_unverifiable`, `problems`, `notes`.
...
No structured result: the tool returns text only.
...
| Argument | Type | Description |
|---|---|---|
| `docs` **(required)** | array of strings | The spec file(s) or folder(s), relative to the project. |
...
Checked on 2026-09-22 against Claude Code 2.1.278, for the spec-drift family.
...
- **Not established:** which permission flags a fully unattended Claude Code run needs before a
  tool that sends will execute. For CI, do not drive an agent — run the scripts directly (exit
  codes 0/1/2/3).
- **Not yet verified in Claude Code:** the ci-triage and code-audit skills' triggering, and their
  tools end to end in a session. They are covered by the server's own tests.

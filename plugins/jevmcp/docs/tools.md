...
- the log of each failed job, at most 40 jobs;
...
| `complete` | boolean | True only when every failure was checked. |
...
- Set a project up in the order: find the source (the spec, or the rule files) → draft the map →
  **review every entry with the user** → validate → with consent, the first run.
...
| `--log FILE [FILE ...]` | Log files from any CI. |
| `--junit FILE [FILE ...]` | JUnit XML reports. |

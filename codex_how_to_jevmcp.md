...
Checked on 2026-09-22 against `codex-cli 0.155.1`, for the spec-drift family.
...
| `codex exec --dangerously-bypass-approvals-and-sandbox '...'` | **Works** (verified: 3 DRIFT on the demo project). It switches off the sandbox *and* all approvals, so use it only where the whole job is already isolated, such as a CI container. |
  ...
  `code_audit.py` (exit codes 0/1/2/3).
- **Not yet verified in Codex:** the ci-triage and code-audit skills' triggering, and their tools
  end to end in a session. They are covered by the server's own tests.

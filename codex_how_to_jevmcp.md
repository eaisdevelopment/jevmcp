<!-- Generated by the maintainers' scripts/build_howto.py from the shipped plugin, jevmcp 1.7.1.
     Verbatim sources:
       plugins/jevmcp/skills/spec-drift/SKILL.md (sha256 5bb49fd6e36a906e...),
       ...
       plugins/jevmcp/scripts/jevmcp_server.py (INSTRUCTIONS and TOOLS; sha256 b94309999f698851...).
...
# OpenAI Codex: the jevmcp instructions

**The instructions are files shipped inside the plugin**: the server's `instructions` and tool list in `plugins/jevmcp/scripts/jevmcp_server.py`, and one skill per tool family — `plugins/jevmcp/skills/spec-drift/SKILL.md`, `plugins/jevmcp/skills/ci-triage/SKILL.md`, `plugins/jevmcp/skills/code-audit/SKILL.md`. OpenAI Codex and the other supported client read the **same bytes**, and nothing inside them is written for one client. The skills' sha256 hashes:

- `spec-drift`: `5bb49fd6e36a906ef495562dbf31a11c7617692f26919c4e1f46bf6a7770d287`
...
What differs is the *envelope*: how the text reaches the model, what the tools are called, whether `project` is mandatory, when approval is asked, and where the API key comes from. That is in [section 5](#5-what-is-different-in-openai-codex), and it is the only reason a second file exists.
...
| 2 | Each skill's frontmatter `description` | spec-drift 825 characters, ci-triage 775 characters, code-audit 788 characters; ~600 tokens in all | at session start, always present |
| 3 | Each skill's body — the procedure | spec-drift 14.0 kB, ci-triage 10.2 kB, code-audit 10.8 kB | only when that skill fires |
| 4 | The 10 tools' descriptions and JSON schemas | 41,682 characters in all | at call time |

In Codex the wrapper is thicker than in Claude Code, and three of its differences change what the agent must actually do: **`project` must be passed on every call**, **every call of a tool that sends asks for approval**, and **the sandbox has no network**, so a command-line fallback inside a session is a silent false pass rather than a fallback. Read section 5 before section 3 if you are debugging a session.
...
(FastAPI, Flask, Django), and with any other language through line ranges.
```
...
### 3.1 `spec-drift` — `plugins/jevmcp/skills/spec-drift/SKILL.md` (sha256 `5bb49fd6e36a906e...`, 14,870 bytes)
...
Structured result fields: `summary`, `project`, `map`, `claims_in_map`, `claims_selected`, `checked`, `counts`, `cost_usd`, `complete`, `results_file`, `map_problems`, `not_checked`, `map_health`, `flagged`, `spec_versions`.
...
> Check that the spec map still fits the spec and the code - every reference resolves, nothing is stale, and (strict, the default) every spec sentence is either mapped or marked excluded with a reason. Also says whether each spec file the map checks is still the current one (spec_versions: one that says it is superseded makes the map not ready; a newer version is a warning to take to the user) and how many entries' sentences moved to another line (moved_entries). Free: sends nothing. Run after editing the spec or the map.
...
Structured result fields: `project`, `map`, `ready`, `entries_to_check`, `excluded`, `full_check_cost_usd`, `full_check_cost_usd_max`, `samples`, `likely_unverifiable`, `problems`, `notes`, `spec_versions`, `moved_entries`.
...
> Show exactly what check_spec_drift would send to TypeSafe for some claims: the sentence, the code with comments removed and secrets redacted, any computed values, and the 3 fixed questions; and whether the spec files the map checks are still current (spec_versions). Free: sends nothing.
...
Structured result fields: `project`, `map`, `model`, `claims`, `spec_versions`.
...
> Set up a project that has no spec map yet, in two calls. First call it WITHOUT docs: it drafts nothing and lists every file that looks like a spec, with its last commit date and flags (looks like an old copy, says it is superseded, a newer version exists). Show that list to the user and ask which file(s) are the current source of truth - never pick one yourself. Then call it with docs = exactly those files and out: it suggests a code location for every sentence and writes a new map file for review, with warnings about any named file that looks old. A folder that holds old copies or several versions of a spec is refused. Every entry must then be reviewed - point 'code' at what enforces the sentence and set status 'reviewed', or set status 'excluded' with a 'why' - before check_spec_drift is worth running. Free: sends nothing. Never overwrites a file.

| Argument | Type | Description |
|---|---|---|
| `docs` | array of strings | The spec file(s) the user named as current, relative to the project. Leave out to list the candidates instead (nothing is written). A folder is refused when it holds old copies or several versions of a spec. |
...
A body is read from disk when its skill fires. The server's `instructions` block is served at
...
triage_ci_failure   preview_ci_triage   check_code_rules   preview_code_audit   validate_rule_map   draft_rule_map
```
...
(`is a home or root folder, not a project`). This is why every skill body says to pass `project` in
every call.

### Approval — **every call of a tool that sends, by design**

`check_spec_drift`, `triage_ci_failure` and `check_code_rules` are write actions
(`readOnlyHint: false`, `openWorldHint: true`) because they send data to `api.typesafe.ai`, so
**Codex prompts before each call**. That prompt *is* the user's consent for that call. Do not try
to route around it.
...
never"*), as designed. The other tools were not called in that run, and an interactive session was
not re-checked for `preview_ci_triage`.

### Network — the trap that matters most

**Codex's sandbox has no network.** A fallback to the command line *inside* a Codex session
therefore reaches nothing: the run cannot call TypeSafe and comes back having found nothing. That
is a **silent false pass**. Use the MCP tools, which work because the MCP server is started outside
the sandbox — and never report an exit-3 run as a pass.
...
- **Log files from another CI** go inside the project, which the agent can always write to. The
  server's private inbox is an alternative, but whether Codex's sandbox lets the agent write there
  was not verified.
...
Checked on 2026-09-22 against `codex-cli 0.155.1`, for the spec-drift family, and on 2026-09-24
against `codex-cli 0.156.1`, for CI triage and `codex exec`.

- **Discovery works with no mention of the plugin: 3/3.** Three neutral prompts loaded the
  spec-drift skill; a control prompt ("What does this project do?") correctly did not.
...
- **CI triage (2026-09-24, 0.156.1, the installed 1.7.0):** `preview_ci_triage` read a failed
  ...
  approval, as designed, and under `codex exec` it was blocked (below).
- **Unattended / CI — exactly what fails.** `codex exec` runs with approval policy `never`:
...
| `codex exec --dangerously-bypass-approvals-and-sandbox '...'` | 0.155.1 | **Works** (verified: 3 DRIFT on the demo project). It switches off the sandbox *and* all approvals, so use it only where the whole job is already isolated, such as a CI container. |
  ...
  With 0.156.1 the tools that send were not run headless: the only unattended route is the last
  row, which switches off approvals and the sandbox. Better for CI: skip the agent and run the
  scripts where the network works — `uv run --script <plugin>/scripts/spec_drift.py --map
  spec_map.json`, `ci_triage.py` or `code_audit.py` (exit codes 0/1/2/3).
- **Not yet verified in Codex:** whether the ci-triage and code-audit skills load from a request
  that does not mention them; the code-audit tools; and a CI triage actually sent from a Codex
